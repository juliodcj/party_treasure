...
| Identificar magia de foco (e truque de foco) | pelo traço `focus` | pela **pasta** `spells/focus/` do pack, gravada como `focus: true` no índice. O traço não serve sozinho: das 545 magias daquela pasta, 52 não o carregam, e 49 dessas são justamente os **truques de foco** (Courageous Anthem e as composições do bardo, os psi do psychic, os hex da bruxa, os do eidolon). Filtrando pelo traço, o app concluía que truque de foco tinha deixado de existir no Remaster — e o jogador ficava sem poder acrescentar um sem reimportar a ficha. Mesmo princípio das ações: quem organiza é a pasta da Paizo. |
| Qual lista de foco recebe cada magia | a que o Pathbuilder mandou (`focusCantrips` × `focusSpells`) | **o corpus decide**, e o export é só o desempate quando o corpus não conhece o nome (conteúdo legado). Export que classifique um truque de composição como magia de foco daria ao personagem um ponto de reserva que a regra não dá. |
